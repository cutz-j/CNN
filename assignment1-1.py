...
def conv_forward(A_prev, W, b, hparameters):
    ...
    for i in range(m):                               # loop over the batch of training examples
        a_prev_pad = A_prev_pad[i,:,:,:]   # 3차원              # Select ith training example's padded activation
        for h in range(n_H-f+1):                           # loop over vertical axis of the output volume
            for w in range(n_W-f+1):                       # loop over horizontal axis of the output volume
                for c in range(n_C):                   # loop over channels (= #filters) of the output volume
                    ...
                    a_slice_prev = a_prev_pad[up_left:up_right, down_left:down_right, :]
        ...
        for h in range(n_H):
            for w in range(n_W):
                for c in range(n_C):
                    ...
                    up_right = h*stride + f
                    down_left = w*stride
                    down_right = w*stride + f
                    
                    # Use the corners to define the current slice on the ith training example of A_prev, channel c. (≈1 line)
                    a_prev_slice = a_prev[up_left:up_right, down_left:down_right, c]
